...
<c:forEach var="s" items="${shipments}" varStatus="idx"> 
<tr ${idx.index%2==0? "style='background-color:grey;color:white'":"" }>
	<td><c:out value="${idx.index}"/></td>
	<td><c:out value="${s.shipmentid}"/></td>
	<td><c:out value="${s.distance}"/></td>
	<td><c:out value="${s.weight}"/></td>
	<td><c:out value="${s.destination}"/></td>
	<td><a href="<c:url value="/shipments/${s.shipmentid}"/>">Detalii</a></td>
	<td><a href="<c:url value="/shipments/edit/${s.shipmentid}"/>">Editeaza</a></td>
	<td><a href="<c:url value="/shipments/delete/${s.shipmentid}"/>">Sterge</a></td>
 </tr>
</c:forEach>
</table>

<button onclick="window.location.href='<c:url value='/shipments/add'/>'">Creaza entitate</button>
<br>
